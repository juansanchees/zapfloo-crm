...
beforeEach(() => {
  ...
  const rows: Record<string, Array<Record<string, unknown>>> = {
    ...
    crm_leads: [before, from.toISOString(), inside, to.toISOString(), after, null].map((closed_at, i) => ({ organization_id: "org-a", owner_user_id: A, value_cents: 10_000 * (i + 1), currency: "BRL", status: "won", closed_at })).concat([{ organization_id: "org-a", owner_user_id: A, value_cents: 99_000, currency: "BRL", status: "lost", closed_at: inside }, { organization_id: "org-b", owner_user_id: A, value_cents: 77_000, currency: "BRL", status: "won", closed_at: inside }]),
    conversations: [before, from.toISOString(), inside, to.toISOString(), after, null].map((assigned_at) => ({ organization_id: "org-a", assigned_to_user_id: A, assigned_at })).concat([{ organization_id: "org-b", assigned_to_user_id: A, assigned_at: inside }]),
  };
  class Q implements PromiseLike<unknown> {
    ...
    then<TResult1 = unknown, TResult2 = never>(onfulfilled?: ((value: unknown) => TResult1 | PromiseLike<TResult1>) | null, onrejected?: ((reason: unknown) => TResult2 | PromiseLike<TResult2>) | null): PromiseLike<TResult1 | TResult2> { const found = (rows[this.table] ?? []).filter((row) => this.ops.every(({op,key,value}) => op === "eq" ? row[key] === value : op === "gte" ? typeof value === "string" && typeof row[key] === "string" && row[key] >= value : typeof value === "string" && typeof row[key] === "string" && row[key] < value)); return Promise.resolve({ data: this.table === "organizations" ? (found[0] ?? null) : found, error: null }).then(onfulfilled, onrejected); }
  }
...
describe("progresso mensal das metas", () => {
  it("executa GET com tenant, won, janela UTC e recorte self", async () => {
    ...
    const conversations = calls.find((call) => call.table === "conversations")!;
    expect(conversations.filters).toEqual(expect.arrayContaining([["organization_id", "org-a"], ["assigned_to_user_id", A]]));
    ...
    expect(conversations.ops).toEqual(expect.arrayContaining([
      { op: "gte", key: "assigned_at", value: body.data.window.from },
      { op: "lt", key: "assigned_at", value: body.data.window.to },
    ]));
  });
  ...
  it.each(["manager", "admin"] as const)("%s recebe roster nominal sem filtros individuais", async (role) => {
    ...
    const body = await (await GET()).json() as { data: { scope: string; members: Array<{ user_id: string; name: string | null }> } };
    ...
    expect(calls.find((call) => call.table === "conversations")!.filters.map(([key]) => key)).not.toContain("assigned_to_user_id");
  });
