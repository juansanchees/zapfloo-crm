...
import { GET, monthWindowInTimeZone } from "./route";
...
beforeEach(() => {
  const window = monthWindowInTimeZone(new Date(), "America/Sao_Paulo");
  const from = new Date(window.from);
  const to = new Date(window.to);
  const before = new Date(from.getTime() - 1).toISOString();
  const inside = new Date(from.getTime() + 86_400_000).toISOString();
  const after = new Date(to.getTime() + 1).toISOString();
  vi.mocked(requireRole).mockResolvedValue({
    ok: true,
    user: { id: A, is_platform_admin: false },
  ...
  const rows: Record<string, Array<Record<string, unknown>>> = {
    organizations: [
      {
        ...
        settings: {
          operational_goals: { currency: "BRL", team: { monthly_revenue_cents: 500_000 } },
        },
      },
    ],
    user_organizations: [
      { organization_id: "org-a", user_id: A, revoked_at: null, accepted_at: "2026-01-01" },
      { organization_id: "org-a", user_id: B, revoked_at: null, accepted_at: "2026-01-01" },
    ],
    crm_leads: (
      [before, from.toISOString(), inside, to.toISOString(), after, null].map((closed_at, i) => ({
        organization_id: "org-a",
        owner_user_id: A,
        value_cents: 10_000 * (i + 1),
        currency: "BRL",
        status: "won",
        closed_at,
    ...
    messages: (
      [before, from.toISOString(), inside, to.toISOString(), after, null].map((sent_at, i) => ({
        organization_id: "org-a",
        direction: "outbound",
        sent_by_user_id: A,
        sent_at,
  ...
  class Q implements PromiseLike<unknown> {
    ...
    eq(k: string, v: unknown) {
      ...
      this.ops.push({ op: "eq", key: k, value: v });
      return this;
    }
    gte(k: string, v: unknown) {
      this.filters.push([k, v]);
      this.ops.push({ op: "gte", key: k, value: v });
      return this;
    }
    lt(k: string, v: unknown) {
      this.filters.push([k, v]);
      this.ops.push({ op: "lt", key: k, value: v });
      return this;
    }
    ...
    not(k: string, operator: string, v: unknown) {
      this.ops.push({ op: `not.${operator}`, key: k, value: v });
      return this;
    }
    order(key: string) {
    ...
    then<TResult1 = unknown, TResult2 = never>(
      ...
      const found = (rows[this.table] ?? []).filter((row) =>
        this.ops.every(({ op, key, value }) =>
          op === "eq"
            ? row[key] === value
            : op === "gte"
              ? typeof value === "string" && typeof row[key] === "string" && row[key] >= value
              : op === "lt"
                ? typeof value === "string" && typeof row[key] === "string" && row[key] < value
                : row[key] !== value,
        ),
      );
      const [from, to] = this.ranges[0] ?? [0, found.length - 1];
      const data = this.table === "organizations" ? (found[0] ?? null) : found.slice(from, to + 1);
...
describe("progresso mensal das metas", () => {
  it("executa GET com tenant, won, janela local da organização e recorte self", async () => {
    ...
    const body = (await response.json()) as {
      data: {
        ...
        members: Array<{
          user_id: string;
          revenue: Array<{ current_cents: number }>;
          conversations: { current: number };
        }>;
      };
    };
    ...
    expect(messages.filters).toEqual(
      expect.arrayContaining([
        ["organization_id", "org-a"],
        ["direction", "outbound"],
        ["sent_by_user_id", A],
  ...
  it("pagina leads e respostas humanas além do limite do PostgREST", async () => {
    const rows = (
      globalThis as typeof globalThis & {
        __goalRows: Record<string, Array<Record<string, unknown>>>;
      }
    ).__goalRows;
    const now = new Date();
    ...
    const calls = (
      globalThis as typeof globalThis & {
        __goalCalls: Array<{ table: string; ranges: Array<[number, number]>; orders: string[] }>;
      }
    ).__goalCalls;
  ...
  it("self usa target individual cifrado, nunca o target da equipe", async () => {
    ...
    vi.mocked(decryptOperationalGoalMembers).mockReturnValue(
      JSON.stringify({
    ...
    const body = (await (await GET()).json()) as {
      data: {
        team: {
          revenue: Array<{ target_cents: number | null }>;
          conversations: { target: number | null };
        };
        members: Array<{ user_id: string; revenue: Array<{ target_cents: number | null }> }>;
      };
    };
  ...
  it.each(["manager", "admin"] as const)(
    "%s recebe roster nominal sem filtros individuais",
    async (role) => {
      vi.mocked(requireRole).mockResolvedValue({
        ok: true,
        user: { id: A, is_platform_admin: false },
        org: { orgId: "org-a", role },
      } as never);
      vi.mocked(nomesDosAtendentes).mockResolvedValue(
        new Map([
          [A, "Ana"],
          [B, "Bia"],
        ]),
      ...
      const body = (await (await GET()).json()) as {
        data: {
          scope: string;
          team: { revenue: Array<{ current_cents: number }>; conversations: { current: number } };
          members: Array<{
            user_id: string;
            name: string | null;
            conversations: { current: number };
      ...
      const calls = (
        globalThis as typeof globalThis & {
          __goalCalls: Array<{ table: string; filters: Array<[string, unknown]> }>;
        }
      ).__goalCalls;
      expect(
        calls.find((call) => call.table === "crm_leads")!.filters.map(([key]) => key),
      ).not.toContain("owner_user_id");
      expect(
        calls.find((call) => call.table === "messages")!.filters.map(([key]) => key),
      ).not.toContain("sent_by_user_id");
    },
  );
  it("platform admin com papel agent recebe o mesmo escopo completo", async () => {
    vi.mocked(requireRole).mockResolvedValue({
      ok: true,
      user: { id: A, is_platform_admin: true },
      org: { orgId: "org-a", role: "agent" },
    } as never);

    const body = (await (await GET()).json()) as {
      data: { scope: string; members: Array<{ user_id: string }> };
    };
    expect(body.data.scope).toBe("team");
        ...
        return {
          select: () => ({
            eq: () => ({
              maybeSingle: async () => ({
                data: {
                  settings: { operational_goals: { currency: "BRL", members_enc: "corrupted" } },
                },
                error: null,
              }),
            }),
  ...
  it("usa intervalo semiaberto no fuso da organização", () => {
    expect(monthWindowInTimeZone(new Date("2026-02-14T12:00:00Z"), "America/Sao_Paulo")).toEqual({
      from: "2026-02-01T03:00:00.000Z",
      to: "2026-03-01T03:00:00.000Z",
    });
  });

  it("cai em Brasília quando a organização ainda não tem um fuso utilizável", () => {
    const expected = {
      from: "2026-02-01T03:00:00.000Z",
      to: "2026-03-01T03:00:00.000Z",
    };
    expect(monthWindowInTimeZone(new Date("2026-02-14T12:00:00Z"), null)).toEqual(expected);
    expect(monthWindowInTimeZone(new Date("2026-02-14T12:00:00Z"), "fuso-inexistente")).toEqual(
      expected,
    );
  });

  it("conta no mês certo um ganho às 22h do último dia em Brasília", async () => {
    vi.useFakeTimers();
    ...
    rows.crm_leads = [
      {
        ...
        currency: "BRL",
        status: "won",
        // 31/03, 22h em Brasília = 01/04 em UTC.
    ...
    const body = (await (await GET()).json()) as {
      data: {
        window: { from: string; to: string };
        members: Array<{ revenue: Array<{ current_cents: number }> }>;
      };
    };
    ...
    expect(body.data.members[0]!.revenue[0]!.current_cents).toBe(42_000);
  });
