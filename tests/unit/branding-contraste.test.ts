...
describe("extrairRegua — os pares saem do globals.css, nunca de lista à mão", () => {
  it("acha os dois temas, a rampa do produto e os neutros", () => {
    ...
    expect(REGUA.escuro.neutros[9]).toBe("#111114");
  ...
  it("reproduz as razões medidas à mão no design system", () => {
    ...
    expect(razao("--color-accent", "--color-bg")).toBeCloseTo(5.53, 2);
    expect(razao(":focus-visible/outline", "--color-bg")).toBeCloseTo(3.81, 2);
    expect(razao(":focus-visible/outline", "--color-surface-elevated")).toBeCloseTo(3.56, 2);
  });
...
describe("marca acromática — o accent do produto permanece", () => {
  ...
  it("o accent que permanece é cromático e separável do neutro do mesmo grau", () => {
    ...
    expect(separacaoDoNeutro(REGUA.claro, marca.claro.grauDoAccent, marca.claro.accent)).toBeCloseTo(0.1075, 4);
    expect(separacaoDoNeutro(REGUA.escuro, marca.escuro.grauDoAccent, marca.escuro.accent)).toBeCloseTo(0.0953, 4);
