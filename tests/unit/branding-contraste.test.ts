...
import { deltaEOklab, hexParaOklch, hexParaRgb, rampaDeSemente } from "@/lib/branding/rampa";
 ...
 *  `#506d48` — marca verde customizada: prova que a rampa white-label continua
 *              independente da nova identidade visual do produto.
 */
const FIXTURE = [
  "#0f172a",
  "#f5c518",
...
describe("extrairRegua — os pares saem do globals.css, nunca de lista à mão", () => {
  it("acha os dois temas, a rampa do produto e os neutros", () => {
    ...
    expect(new Set(REGUA.rampaDoProduto).size).toBe(11);
    expect(new Set(REGUA.escuro.neutros).size).toBe(11);
  ...
  it("classifica -fg como texto e -soft como superfície", () => {
    ...
    expect(REGUA.escuro.tingidas[0]?.fonte).toMatchObject({ tipo: "pino", indice: 5 });
    expect(REGUA.escuro.indices.soft).toBe(5);
    expect(REGUA.escuro.alfaDoSoft).toBeCloseTo(0.16, 6);
  });

  it("mede o pino literal do produto e o remapeia para a rampa white-label", () => {
    const papel = REGUA.escuro.papeis.find((p) => p.token === "--color-accent");
    ...
    expect(pinoEmpatado).toMatchObject({ tipo: "pino", indice: 4 });
  });
  ...
  it("reproduz as razões medidas à mão no design system", () => {
    ...
    expect(razao("--color-accent", "--color-bg")).toBeCloseTo(4.17, 2);
    expect(razao(":focus-visible/outline", "--color-bg")).toBeCloseTo(4.17, 2);
    expect(razao(":focus-visible/outline", "--color-surface-elevated")).toBeCloseTo(3.93, 2);
  });
...
describe("derivarMarca — as 16 sementes adversariais", () => {
  ...
  it("a caminhada de contraste de fato ANDA — e nas sementes previstas", () => {
    ...
    // No escuro o pino ocupa a âncora 500 e a caminhada segue a partir dela.
    expect(amarelo.escuro.accent).toBe(amarelo.rampa[7]);
  });
...
describe("reconciliação — quem se move são as NOSSAS semânticas", () => {
  it("uma colisão literal continua alcançando a reconciliação (controle positivo)", () => {
    const success = REGUA.escuro.semanticas.find((s) => s.nome === "success");
    expect(success).toBeDefined();
    const reconciliada = reconciliarSemanticas(success!.hex, REGUA.escuro.semanticas);
    expect(reconciliada.semSaida).toContainEqual({ nome: "success", separacao: 0 });
    expect(reconciliada.movimentos.map((m) => m.nome)).toEqual(["warning", "error"]);
  });
  ...
  it("toda semântica movida folga do accent e continua dentro do orçamento de rotação", () => {
    ...
    // Guarda de vacuidade do run inteiro, medida com a paleta Nocturne.
...
describe("marca acromática — o accent do produto permanece", () => {
  ...
  it("o accent que permanece é cromático e separável do neutro do mesmo grau", () => {
    ...
    // Medição após a mudança intencional para superfícies grafite. Os pisos
    // acima continuam inalterados; estes valores caracterizam a nova paleta.
    expect(
      separacaoDoNeutro(REGUA.claro, marca.claro.grauDoAccent, marca.claro.accent),
    ).toBeCloseTo(0.0964, 4);
    expect(
      separacaoDoNeutro(REGUA.escuro, marca.escuro.grauDoAccent, marca.escuro.accent),
    ).toBeCloseTo(0.0978, 4);
