...
describe("fitToBudget — mídia no contexto", () => {
  ...
  it("mídia com derivado: conteúdo presente + enquadrado (agente não nega a mídia)", () => {
    const ctx = __test_fitToBudget(base, [
      { direction: "inbound", type: "audio", body: null, media_url: "u", media_storage_path: "p", media_mime: "audio/ogg", media_derived_text: "quero o tênis 42", media_derived_status: "ready", sent_at: new Date("2026-07-22T10:00:00Z") },
    ], 100000, "UTC");
  ...
  it("mídia SEM derivado ainda cai no marcador [tipo]", () => {
    const ctx = __test_fitToBudget(base, [
      { direction: "inbound", type: "image", body: null, media_url: "u", media_storage_path: "p", media_mime: "image/jpeg", media_derived_text: null, media_derived_status: "pending", sent_at: new Date("2026-07-22T10:00:00Z") },
    ], 100000, "UTC");
  ...
  it("texto puro inalterado", () => {
    const ctx = __test_fitToBudget(base, [
      { direction: "inbound", type: "text", body: "oi", media_url: null, media_storage_path: null, media_mime: null, media_derived_text: null, media_derived_status: null, sent_at: new Date("2026-07-22T10:00:00Z") },
    ], 100000, "UTC");
  ...
  it("mídia com LEGENDA e derivado — os dois coexistem, legenda não mascara a descrição", () => {
    const ctx = __test_fitToBudget(base, [
      { direction: "inbound", type: "image", body: "quero esse", media_url: "u", media_storage_path: "p", media_mime: "image/jpeg", media_derived_text: "tênis vermelho tamanho 42", media_derived_status: "ready", sent_at: new Date("2026-07-22T10:00:00Z") },
    ], 100000, "UTC");
