...
const f = vi.hoisted(() => ({ ctx: vi.fn(), rpc: vi.fn(), from: vi.fn(), providers: vi.fn() }));
...
vi.mock("@/lib/ai/credenciais/gerenciada", () => ({ provedoresComCredencialGerenciada: f.providers }));
...
function dadosDoBanco(
  value: typeof draft | null,
  models: Array<{ provider: string; model_id: string; display_name: string }> = [],
) {
  f.from.mockImplementation((table: string) => {
    const result = { data: table === "onboarding_drafts" ? value : table === "ai_agents" ? { is_active: false, is_default: false, published_version_id: null, archived_at: "2026-09-10T12:00:00Z" } : table === "ai_models" ? models : [], error: null };
...
beforeEach(() => { vi.clearAllMocks(); f.ctx.mockResolvedValue({ orgId: org, userId: user }); f.providers.mockResolvedValue([]); f.rpc.mockResolvedValue({ data: null, error: { message: "draft_unavailable" } }); dadosDoBanco(draft); });
describe("leitura expõe recuperação sem inventar prova de ensaio", () => {
  ...
  it("preparação indisponível deixa painel acessível e sem prova", async () => {
    expect(await lerEnsaio({ expected_context: context })).toEqual({ ok: true, panel: { selection: { revision: 1, agent_id: user, version_id: version, provider: "openai", model: "qa", credential_id: null }, proof: null, models: [], recovery_available: true } });
  ...
  it("FK apagada conserva revisão preparada como sinal de recuperação", async () => {
    ...
    expect(await lerEnsaio({ expected_context: context })).toEqual({ ok: true, panel: { selection: null, proof: null, models: [], recovery_available: true } });
    expect(f.rpc).not.toHaveBeenCalled();
  });
  it("rascunho novo não oferece recuperação nem prova", async () => {
    dadosDoBanco(null);
    expect(await lerEnsaio({ expected_context: context })).toEqual({ ok: true, panel: { selection: null, proof: null, models: [], recovery_available: false } });
  });
  it("catálogo do ensaio contém somente provedores que têm chave utilizável", async () => {
    f.providers.mockResolvedValue(["anthropic"]);
    dadosDoBanco(null, [
      { provider: "openai", model_id: "gpt-5.6-luna", display_name: "Luna" },
      { provider: "anthropic", model_id: "claude-haiku-4-5", display_name: "Haiku" },
    ]);

    const result = await lerEnsaio({ expected_context: context });
    expect(result.ok && result.panel.models).toEqual([
      { provider: "anthropic", model_id: "claude-haiku-4-5", display_name: "Haiku" },
    ]);
  });
});
